...
/**
 * The {@link ServletTestSuite} is a suite of junit tests touching the health check operations
 *  via the HTTP interface. This suite is used to blackbox test the operations of the servlet-specific
...
public abstract class ServletTestSuite extends TestSuite {
    ...
    protected void assertNotMatches ( final String url, final String responseText, final Pattern pattern ) {
        if ( pattern.matcher(responseText).find() ) {
            final String message = "Expected not to match pattern '" + pattern.pattern() + "' at '" + url + "'.";
            log.error(message);
    ...
    protected void assertMatches ( final String url, final String responseText, final Pattern pattern ) {
        if ( !pattern.matcher(responseText).find() ) {
            final String message = "Expected to match pattern '" + pattern.pattern() + "' at '" + url + "'.";
            log.error(message);
